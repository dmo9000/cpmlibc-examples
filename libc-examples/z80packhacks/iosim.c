...
void fdump(FILE *fh, BYTE *memptr[], int bytes);
...
void init_io(void)
{
	...
	if (stat("/tmp/.z80pack", &sbuf) != 0) {
		...
		mknod("/tmp/.z80pack/auxin", 0666 | S_IFIFO, 0);
		mknod("/tmp/.z80pack/auxout", 0666 | S_IFIFO, 0);
	}
	...
	if ((auxin = open("/tmp/.z80pack/auxin", O_RDONLY | O_NDELAY)) == -1) {
		perror("pipe auxin");
		exit(1);
	}
	if ((auxout = open("/tmp/.z80pack/auxout", O_WRONLY)) == -1) {
		perror("pipe auxout");
		exit(1);
	}
#endif
...
static void fdco_out(BYTE data)
{
 ...
 // printf("FDCT_OUT(%u)\r\n", data);
  ...
  /* DISK IO CODE GOES HERE */
	iolog = fopen("zeropage.log", "a+");
	//fprintf(iolog, "memory = 0x%0x\n", memory);
	fdump(iolog, memory, 256);
	switch (data) {
	case 0:	/* read */
...
/*
	...
	for (i = 0; i < bytes; i++) {
     ...
     if (i < bytes && !c) {
        fprintf(fh, "%x: ", memptr[0] + i);
        }

		fprintf(fh, "%02x ", memptr[0][i]);
		if (memptr[0][i] >= 32 && memptr[0][i] <= 127) {
			ascbuf[c] = memptr[0][i];
			} else {
			ascbuf[c] = '.';
			}
	...
	*/
	fcb_ptr = memptr[0] + 0x5c;
//	fprintf(fh, "fcb_ptr = 0x%0x\n", fcb_ptr);
	//fprintf(fh, "fcb_ptr->filename = %s, ", fcb_ptr->filename);
	fprintf(fh, "fcb_ptr->ex = 0x%02x, ", fcb_ptr->ex);
	fprintf(fh, "fcb_ptr->resv = 0x%04x\n", fcb_ptr->resv);
//	fprintf(fh, "fcb_ptr->filetype = %s\n", fcb_ptr->filetype);
