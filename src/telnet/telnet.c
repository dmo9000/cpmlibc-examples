...
#define ZMODEM_MAGIC "rz""\x0d""**""\x18"
...
int output_byte(int type, int s, uint8_t b, const char *nb, int nboffs, int nbsize)
{
    int i = 0;
    ...
    if (memcmp(&ring_buffer, (const char *) ZMODEM_MAGIC, 6) ==0) {
        printf("\n *** ZMODEM SIG DETECTED at netbuf offset %d/%d ***\n", nboffs, nbsize);
        if (zmodem_detect(s, (const char *) &ring_buffer, RING_BUFFER_SIZE, nb, nboffs, nbsize)) {
        ...
        for (i = 0; i < rc; i++) {
            ...
            case false:
                ...
                case 0xFF:
                    ...
                    break;
                default:
                    if (output_byte(OUTPUT_BINARY, s, netbuf[i], (const char *) &netbuf, i, rc) == 0) {
