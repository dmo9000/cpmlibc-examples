...
/*
int memputch(const char *s, char c)
{
    char *p = (char *) s;
    p[0] = c;
    p[1] = '\0';
...
int zprintf(const char *str, const char *fmt, ...) {
    register int c = 0;
    int count = 0;
    int type = 0, base = 10;
    long val = 0;
    ...
    while (c = *fmt++) {
        ...
        if (c != '%') {
            ...
            case 'x':
                ...
                cprintf("{val = %u, type = %u, base = %u}\n", val, type, base);
                cp = __numout(val, base, out2);
                if (0) {
                case 's':
                    cp = va_arg(ap, char *);
                }
